...
async def profile_performance_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if employee_data:
        for emp_id, data in employee_data.items():
            ...
            gross_commission = data.get('scaled_commission', data['commission'] - data.get('actual_base_salary', 0) - data.get('incentive', 0) + loss_bear)
            ...
            total_income = gross_commission - loss_bear + actual_base + incentive + channel_fee + tech_fee + response_award

            # 明细行（最后一行用 └─，其余用 ├─）
            completion_rate = data.get('completion_rate', 0)
            has_month_tasks = data.get('has_month_tasks')
            perf_note = ""
            if has_month_tasks:
                perf_note = f"，完成率 {completion_rate}%"
            details = [f"业绩：{data['performance']:.2f} USDT（提成：{gross_commission:.2f} USDT{perf_note}）"]

            base_salary = data.get('base_salary', 0)
            if base_salary > 0:
                details.append(f"底薪：{base_salary:.2f} USDT")
            ...
            if response_award > 0:
                ...
                beaten = data.get('response_award_beaten')
                total_ref = data.get('response_award_total_ref')
                award_rank = data.get('response_award_rank', 1)
                cap_note = ""
                if award_cap > 0:
                    cap_note = "，已达上限" if data.get('response_award_capped') else f"，上限 {award_cap:g}"
                if beaten is not None and total_ref:
                    details.append(f"响应速度奖：{response_award:.2f} USDT（公司总收益 × {award_rate*100:.1f}%，超过 {beaten}/{total_ref} 参考人{cap_note}）")
                else:
                    details.append(f"响应速度奖：{response_award:.2f} USDT（公司总收益 × {award_rate*100:.1f}%，响应速度第{award_rank}名{cap_note}）")
    ...
    if (settings.get('response_award_rate') or 0) > 0:
        ...
        ref_names = company_fees.get('response_award_ref_names', '')
        unq_names = company_fees.get('response_award_ref_unqualified_names', '')
        unq_text = f"，其中{unq_names}次数未达标" if unq_names else ""
        winner_name = company_fees.get('response_award_emp_name', '')
        if ref_names:
            fee_parts.append(f"响应速度奖：{resp_award_rate*100:.1f}%（参考人：{ref_names}{unq_text}{cap_text}）")
        elif winner_name:
    ...
    if data == "perf_export_all":
        ...
        for m in months:
            ...
            for k in ('channel_fee_emp', 'tech_fee_emp', 'response_award_emp',
                      ...
                      'response_award_ref_unqualified_names', 'response_award_mode'):
                if mf.get(k) not in (None, 0, '', False, []):
                    company_fees[k] = mf[k]
                ...
                for k, v in emp.items():
                    if k in ('is_channel', 'is_customer', 'has_month_tasks', 'response_award_capped'):
                        dst[k] = dst.get(k, False) or v
                            ...
                            'completion_rate', 'completion_factor', 'incentive_threshold', 'incentive_rate',
                            'response_award_rank', 'channel_fee_rate', 'tech_fee_rate',
                            'response_award_rate', 'response_award_cap',
                            'response_award_beaten', 'response_award_total_ref'):
                        dst[k] = dst.get(k, 0) + v
...
"""
    for emp_id, data in employee_commission.items():
        ...
        gross_commission = emp_data.get('scaled_commission', emp_data.get('commission', data['commission']) - emp_data.get('actual_base_salary', 0) - emp_data.get('incentive', 0) + loss_bear)
        ...
        has_month_tasks = emp_data.get('has_month_tasks')
        perf_note = f"，完成率 {completion_rate}%" if has_month_tasks else ""
        details = f"<span class=\"employee-perf\">业绩 {perf:.2f} USDT（提成 {gross_commission:.2f} USDT{perf_note}）</span>"

        if base_salary > 0:
            details += f"<span class=\"employee-base\">底薪 {base_salary:.2f} USDT</span>"
        ...
        if response_award > 0:
            ...
            beaten = emp_data.get('response_award_beaten')
            total_ref = emp_data.get('response_award_total_ref')
            award_rank = emp_data.get('response_award_rank', 1)
            cap_note = ""
            if award_cap > 0:
                cap_note = "，已达上限" if emp_data.get('response_award_capped') else f"，上限 {award_cap:g}"
            if beaten is not None and total_ref:
                details += f"<span class=\"employee-fee\">响应速度奖 {response_award:.2f} USDT（公司总收益 × {award_rate*100:.1f}%，超过 {beaten}/{total_ref} 参考人{cap_note}）</span>"
            else:
                details += f"<span class=\"employee-fee\">响应速度奖 {response_award:.2f} USDT（公司总收益 × {award_rate*100:.1f}%，响应第{award_rank}名{cap_note}）</span>"
...
"""
    ...
    if (settings or {}).get('response_award_rate', 0) > 0:
        ...
        winner_name = company_fees.get('response_award_emp_name', '')
        if ref_names:
            fee_settings_parts.append(f"响应速度奖：{resp_award_rate*100:.1f}%（参考人：{ref_names}{unq_text}{cap_text}）")
        elif winner_name:
