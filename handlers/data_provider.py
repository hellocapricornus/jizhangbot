...
class DataProvider:
    ...
    def get_today_active_groups(self) -> Dict:
        ...
        if not active_groups:
            return {"message": "今日没有群组有交易记录", "active_groups": [], "total_active": 0}
    ...
    def get_address_stats(self, address: str, date_range: str = "today") -> Dict:
        """获取地址的收支统计"""
    ...
    def get_address_monthly_stats(self, address: str) -> Dict:
        """获取地址的月度统计"""
        from handlers.monitor import get_address_balance, get_monthly_stats

        balance = run_async(get_address_balance(address))
        monthly_stats = run_async(get_monthly_stats(address))
