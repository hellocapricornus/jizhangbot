...
FEE_RATE_INPUT, FEE_CAP_INPUT, FEE_RESPONSE_MENU, FEE_REF_SELECT = range(4)
...
def _render_company_fee_menu(settings):
    ...
    resp_ref_raw = (settings.get('response_award_reference_ids') or '').strip()
    resp_ref_names = []
    if resp_ref_raw:
        for rid in resp_ref_raw.split(','):
            rid = rid.strip()
            if rid.isdigit():
                resp_ref_names.append(_emp_name(int(rid)))
    ...
    if resp_rate > 0:
        ...
        if resp_ref_names:
            resp_text += f"｜参考人：{'、'.join(resp_ref_names)}"
    else:
        resp_text = "未设置"
    ...
    text = (
        ...
        f"响应速度奖：{resp_text}\n\n"
        "费用金额 = 公司总收益 × 比例\n"
        "响应速度奖：设置参考人后，参评员工响应次数达到全体平均值，再按击败参考人数比例获奖（击败全部=全额，击败半数=50%；参考人自己次数未达标视为自动被超过）；未设参考人则发给当月响应第一名"
    )
    ...
    if emp_key is None:
        ...
        if ref_raw:
            for rid in ref_raw.split(','):
                rid = rid.strip()
                if rid.isdigit():
                    op_info = operators.get(int(rid), {})
                    ref_names.append(op_info.get('first_name', op_info.get('username', f"员工{rid}")))
        ref_text = '、'.join(ref_names) if ref_names else "未设置（使用第一名逻辑）"
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("设置参考人", callback_data='employee_feecfg_response_ref')],
        ...
        await query.edit_message_text(
            ...
            f"当前上限：{cap_text}\n"
            f"当前参考人：{ref_text}\n\n"
            "奖励 = 公司总收益 × 比例；参评员工响应次数需达到全体平均值，再按击败参考人数比例获奖（击败全部=全额，击败1/2=50%）；参考人自己次数未达平均时视为自动被超过，最高不超过上限",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode='Markdown'
        )
...
async def fee_response_menu(update: Update, context: CallbackContext):
    ...
    if query.data == 'employee_feecfg_response_ref':
        # 进入参考人多选
        settings = get_performance_settings()
        ref_raw = (settings.get('response_award_reference_ids') or '').strip()
        selected = set()
        ...
        context.user_data['response_ref_selected'] = selected
        return await _render_ref_select(update, context)

    # 返回公司费用菜单
    return await company_fee_menu(update, context)


async def _render_ref_select(update: Update, context: CallbackContext):
    """渲染参考人多选界面"""
    query = update.callback_query
    selected = context.user_data.get('response_ref_selected', set())
    operators = list_operators()

    keyboard = []
    for op_id, op_info in operators.items():
        name = op_info.get('first_name', f"员工{op_id}")
        mark = "✅ " if op_id in selected else "⬜ "
        keyboard.append([InlineKeyboardButton(f"{mark}{name}", callback_data=f'employee_fee_ref_toggle_{op_id}')])

    sel_names = []
    for sid in sorted(selected):
        op_info = operators.get(sid, {})
        sel_names.append(op_info.get('first_name', f"员工{sid}"))
    sel_text = '、'.join(sel_names) if sel_names else "无（将使用第一名逻辑）"

    keyboard.append([InlineKeyboardButton("清空全部", callback_data='employee_fee_ref_clear')])
    keyboard.append([InlineKeyboardButton("✅ 完成", callback_data='employee_fee_ref_done'),
                     InlineKeyboardButton("⬅️ 返回", callback_data='employee_feecfg_response')])

    text = (
        "⚙️ 设置响应速度奖参考人\n\n"
        f"已选：{sel_text}\n\n"
        "点击员工切换选择。参考人作为响应速度的标杆，其他员工击败几名参考人就拿几分之几的奖励。"
    )
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    return FEE_REF_SELECT


async def fee_ref_toggle(update: Update, context: CallbackContext):
    """切换参考人选择"""
    query = update.callback_query
    await query.answer()
...
async def fee_ref_clear(update: Update, context: CallbackContext):
    """清空参考人选择"""
    query = update.callback_query
    await query.answer()
    context.user_data['response_ref_selected'] = set()
    return await _render_ref_select(update, context)


async def fee_ref_done(update: Update, context: CallbackContext):
    """保存参考人设置"""
    query = update.callback_query
    await query.answer()

    selected = context.user_data.get('response_ref_selected', set())
    ref_ids_str = ','.join(str(i) for i in sorted(selected))

    ok = _save_fee_settings('response', None, 0, update.effective_user.id,
                            response_award_reference_ids=ref_ids_str)
    context.user_data.pop('response_ref_selected', None)
    context.user_data.pop('fee_type', None)

    if ok:
        if selected:
            operators = list_operators()
            names = [operators.get(i, {}).get('first_name', f"员工{i}") for i in sorted(selected)]
            await query.message.reply_text(f"✅ 响应速度奖参考人已设置：{'、'.join(names)}")
        else:
            await query.message.reply_text("✅ 已清空响应速度奖参考人（将使用第一名逻辑）")
        # 返回公司费用设置菜单
        return await company_fee_menu(update, context)

    await query.message.reply_text("❌ 设置失败")
    return ConversationHandler.END


def _save_fee_settings(fee_type, rate, emp_id, user_id, response_award_cap=None, response_award_reference_ids=None):
    """保存某项费用设置（保留其他费用类型的原设置；None 表示保留原值）"""
    settings = get_performance_settings()
    ch_emp = int(settings.get('channel_fee_employee_id') or 0)
    ch_rate = settings.get('channel_fee_rate') or 0
    tech_emp = int(settings.get('tech_fee_employee_id') or 0)
    ...
    resp_rate = settings.get('response_award_rate') or 0
    resp_cap = settings.get('response_award_cap') or 0
    resp_ref = settings.get('response_award_reference_ids') or ''
    ...
    if fee_type == 'channel':
        ...
        if response_award_reference_ids is not None:
            resp_ref = response_award_reference_ids

    return update_company_fee_settings(ch_emp, ch_rate, tech_emp, tech_rate, resp_rate, user_id, resp_cap, resp_ref)
...
def register_employee_handlers(application):
    ...
    fee_conv = ConversationHandler(
        ...
        states={
            ...
            FEE_RESPONSE_MENU: [
                CallbackQueryHandler(fee_response_menu, pattern='^employee_feecfg_response_(rate|cap|ref)$'),
                CallbackQueryHandler(company_fee_menu, pattern='^employee_company_fee$'),
            ],
            FEE_REF_SELECT: [
                CallbackQueryHandler(fee_ref_toggle, pattern='^employee_fee_ref_toggle_'),
                CallbackQueryHandler(fee_ref_clear, pattern='^employee_fee_ref_clear$'),
                CallbackQueryHandler(fee_ref_done, pattern='^employee_fee_ref_done$'),
                CallbackQueryHandler(fee_config_menu, pattern='^employee_feecfg_response$'),
            ],
        },
