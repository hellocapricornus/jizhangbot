            ...
            with self._get_conn() as conn:
                ...
                query = """
                    SELECT r.record_type, r.amount, r.amount_usdt, r.description, r.created_at, 
                           r.username, r.category, r.user_id, u.first_name, r.rate
                ...
                for row in rows:
                    record = {
                        ...
                        'category': row[6],
                        'user_id': row[7],
                        'rate': row[9] if len(row) > 9 else 0,  # 添加 rate 字段
                    }
            ...
            with self._get_conn() as conn:
                ...
                c.execute("""
                    ...
                    (group_id, session_id, user_id, username, record_type, amount, amount_usdt, 
                     description, category, rate, created_at, date)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (group_id, session['session_id'], user_id, username, record_type, amount, 
                      amount_usdt, description, category, actual_rate, now, date_str))
                conn.commit()
            return True
        except Exception as e:
    ...
    if income_records:
        ...
        if no_category_records:
            for r in no_category_records[:MAX_DISPLAY_RECORDS]:
                ...
                # 使用保存的汇率（加粗显示，整数不显示小数点）
                rate = r.get('rate', 0)
                if rate == int(rate):
                    rate_info = f" **/{int(rate)}**" if rate > 0 else ""
                else:
                    rate_info = f" **/{rate:.2f}**" if rate > 0 else ""
                ...
                message += f"  {time_str} {amount_str}{rate_info} = {amount_usdt:.2f} USDT{mention}\n"
...
async def handle_date_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if income_records:
        ...
        for r in display_income:
            ...
            rate = r.get('rate', 0)
            if rate == int(rate):
                rate_info = f" /{int(rate)}" if rate > 0 else ""  # 去掉 **
            else:
                rate_info = f" /{rate:.2f}" if rate > 0 else ""  # 去掉 **
            ...
            amount_str = f"{amount:+.2f}"
            message += f"\n`{time_str} {amount_str}{rate_info} = {amount_usdt:.2f} USDT`{mention}"
